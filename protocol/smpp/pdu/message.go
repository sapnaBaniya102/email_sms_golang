...
import (
	...
	"bytes"
	"encoding/hex"
	coding3 "github.com/sujit-baniya/smpp/coding"
	"io"
	"reflect"
)
...
func (p *ShortMessage) ReadFrom(r io.Reader) (n int64, err error) {
	...
	if p.DataCoding != coding3.NoCoding {
		coding, _ := buf.ReadByte()
		p.DataCoding = coding3.DataCoding(coding)
	}
...
func (p *ShortMessage) Compose(input string) (err error) {
	coding := coding3.BestCoding(input)
	length := coding.Splitter().Len(input)
	if length > MaxShortMessageLength {
